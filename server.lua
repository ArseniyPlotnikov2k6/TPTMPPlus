	...
	function handler(id,client)
		...
		while 1 do
			...
			elseif cmd==21 then
				...
				elseif client.room == "null" and contains(config.admin,client.nick) == false then
					serverMsg(client, "You can't kick people from the lobby")
				elseif rooms[client.room][1] ~= id and contains(config.admin,client.nick) == false then
